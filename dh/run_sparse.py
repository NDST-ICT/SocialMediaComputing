...
while step * batch_size < training_iters:
	...
	# print lambda1
        ...
        out_put[step//display_step] = [np.mean(train_loss), np.mean(val_loss), np.mean(test_loss), best_val_loss, best_test_loss]
        train_loss = []
        patience -= 1
        if not patience:
            break
...
out_result = [len(predict_result), len(y_test), time.time()-start, best_val_loss, best_test_loss, lambda1]
pickle.dump((out_put, out_result), open("result_old"+".pkl",'wb'))

print len(predict_result),len(y_test)
print "Finished!\n----------------------------------------------------------------"
print "Time:", time.time()-start
print "Time of every training is ",time_p
